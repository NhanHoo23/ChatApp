    ...
    func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        ref.observeSingleEvent(of: .value, with: {[weak self]snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                //convertions array exists for current user
                //should append
...
//        }
        ...
        print("⭐️ Adding convo \(conversationID)")
        ...
        database.child("\(id)/messages").observe(.value, with: {snapshot in
            ...
            let messages: [Message] = value.compactMap({dictionary in
                guard
                    ...
                    print("⭐️ error to get all message")
                    return nil
                }
                ...
                if type == "photo" {
                    ...
                    guard
                        let longtitude = Double(locationComponents[0]),
                        let latitude = Double(locationComponents[1])
                    else {
                        return nil
                    }
                    
                    print("⭐️ rendering location: long-\(longtitude), latitude-\(latitude)")
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: {snapshot in
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations, withCompletionBlock: {error, _ in
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: {snapshot in
                            ...
                            guard let currentName = UserDefaults.standard.value(forKey: "email") as? String else {return}
    ...
    public func deleteConversation(conversationID: String, completion: @escaping(Bool) -> Void) {
        ...
        print("⭐️ deleting conversation with id: \(conversationID)")
        
        //get all conversations for current user
        // delete conversation in collection with target id
        // reset those coversaitons for the user in database
            ...
            if var conversations = snapshot.value as? [[String: Any]] {
                ...
                for conversation in conversations {
                    if let id = conversation["id"] as? String,
                       id == conversationID {
                        print("⭐️ found conversation to delete")
                        break
                    }
                    positionToRemove += 1
                }
                ...
                ref.setValue(conversations, withCompletionBlock: {error, _ in
                    guard error == nil else {
                        completion(false)
                        print("⭐️ failed to write new conversatino array")
                        return
                    }
                    print("⭐️ delete conversation")
                    completion(true)
                })
            }
        })
    ...
    public func conversationExist(with targetRecipientEmail: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        guard
            let senderEmail = UserDefaults.standard.value(forKey: "email") as? String
        else {
            return
        }
...
struct ChatAppUser {
    ...
    var prfilePictureFileName: String {
